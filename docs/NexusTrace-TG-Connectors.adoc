...
* MIPI20 pin#12 RTCK signal is not applicable to RISC-V. If the target is not providing a trace, the target connector should provide GND on this pin. If trace is used, pin#12 should be used as TRC_CLK signal.
...
* Defining MIPI20 pins #11 and #13 as optional TgtPwr pins (to supply 5V to power-up a small, evaluation target board).
...
== Rationale (looking at existing standards)
...
** Based on MIPI 20-pin connector (defined by MIPI) - this is for medium-performance tracing (4-bit, 100+ MHz double edge captures, max trace bandwidth 800Mbps).
...
* In July 2021 MIPI Alliance (following recommendations by Nexus TG group) released White Paper updating recommendations for debug and trace connectors and allowing trace via MIPI20 connector. 

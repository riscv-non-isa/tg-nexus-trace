...
2022/12/5: Version 0.9.8-Stable
...
* Make all tables with automatic widths (to be manually adjusted later)

* Updates after discussion by email
** Explicitly enforcing one encoder for each hart
** Renamed 'ATB Sink' to 'ATB Out Sink' to clarify purpose
** Moved 'trTeWrapOnFull' to 'trRamControl'
** Using 'core/hart' instead of 'core' (term 'hart' only would not be very clear)
** Clarifying reserved registers
...
Instruction Trace is a system that collects a history of processor execution, along with other events. The trace system may be set up and controlled using a register-based interface. Core/hart execution activity appears on the Ingress Port and feeds into a Trace Encoder where it is compressed and formatted into trace messages. The Trace Encoder transmits trace messages to a Trace Sink. In multi-core systems, each core/hart has its own Trace Encoder, and typically all will connect to a Trace Funnel that aggregates trace data from multiple sources and sends the data to a single destination.

This specification does not define the hardware interconnection between the core/hart and Trace Encoder, as this is defined in the _Efficient Trace for RISC-V Specification Version 2.0_. This document also does not define the hardware interconnection between the Trace Encoder and Trace Funnel, or between the Trace Encoder/Funnel and Trace Sink.
...
*Trace Encoder* - Hardware module that accepts execution information from a core/hart and generates a stream of trace messages/packets.
...
|Trace Encoder |0x1=TRCOMP_ENCODER|trBaseEncoder|Accepts execution information from a core/hart (via Trace Ingress Port) and generates a stream of RISC-V trace messages/packets.
...
|Trace ATB Out Sink |0xB=TRCOMP_ATBOUTSINK|trBaseAtbOutSink|Accepts RISC-V trace messages/packets (from Trace Encoder or Trace Funnel) and outputs them to ATB bus in a system.
|===
...
* Memory accesses through RISC-V debug module using SBA (System Bus Access) as defined in RISC-V Debug Specification. Using SBA access (via debug module) is essential to make sure that trace registers are accessible while core/hart is running. It allows external debugger to monitor trace status and to control trace captures without stopping the core/hart being traced.
...
|0x010 - 0xFFF |-- |Optional |Additional registers (specific for particular type of component). All not used registers are reserved and should read as 0 and ignore writes.
|===
...
*Trace ATB Out Sink Registers (trAtbOut...)*

//[cols="15%,20%,10%,~",options="header",]
[cols="~,~,~,~",options="header",]
|===
|*Address Offset* |*Register Name* |*Compliance* |*Description*
|0x000 |trAtbOutControl |Required |Trace ATB Out Sink control register
|0x004 |trAtbOutImpl |Required |Trace ATB Out Sink Implementation information
|===
...
The ATB Out Sink transmits bytes of RISC-V trace messages as an ATB bus master.
...
|12  |trTeInstStallOrOverflow |Written to 1 by hardware when an overflow message is generated or when the TE requests a core/hart stall. Clears to 0 at TE reset or when trace is enabled (trTeEnable set to 1).|R |0
|13 |trTeInstStallEn |
0: If TE cannot send a message, an overflow is generated when trace is restarted. 

1: If TE cannot send a message, the core/hart is stalled until it can.
...
2: Count core/hart clock cycles
...
In a system with an Internal Core timestamp counter (implemented in Trace Encoder associated with a core/hart), it may be desirable to stop the counter when the hart is halted by a debugger. An optional control bit is provided for this purpose, but it may or may not be implemented.
...
Debug triggers are signals from the core/hart that a trigger (breakpoint or watchpoint) was hit, but the action associated with that trigger is a trace-related action. Action identifiers 2-5 are reserved for trace actions in the RISC-V Debug Spec, where triggers are defined. Actions 2-4 are defined by the Efficient Trace for RISC-V (E-Trace) Specification. The desired action is written to the action field of the Match Control mcontrol CSR (0x7a1). Not all cores/harts support trace actions; the debugger should read back mcontrol CSR after setting one of these actions to verify that the option exists.
...
|4 |trRamStopOnWrap |1: Disable capture (trRamEnable -> 0) when circular buffer fills for the first time. |WARL |0
|===
...
|===
|*Bit* |*Field* |*Description* |*RW* |*Reset*
|0 |teWrap |Set to 1 by hardware when trRamWP wraps. It is only set to 0 if trRamWPLow is written|WARL |0
...
Some SoCs may have an Advanced Trace Bus (ATB) infrastructure to manage trace produced by other components. In such systems, it is feasible to route RISC-V trace output to the ATB through an ATB Out Trace Sink. This module manages the interface to ATB, generating ATB trace records that encapsulate RISC-V trace produced by the Trace Encoder or Trace Funnel. There is a control register that includes trace on/off control and a field allowing software to set the Trace Source ID to be used on the ATB. This Trace Source ID allows software to extract RISC-V trace from the combined trace. This interface is compatible with AMBA 4 ATB v1.1.

*Register: trAtbOutBase+0x000 atbControl: ATB Out Sink Control Register*

//[cols="10%,24%,~,8%,8%",options="header",]
[cols="~,~,~,~,~",options="header",]
|===
|*Bit* |*Field* |*Description* |*RW* |*Reset*
|0 |trAtbOutActive |Primary enable for the ATB Out Sink. When 0, the ATB Out Sink may have clocks gated off or be powered
down, and other register locations may be inaccessible. Hardware may take an arbitrarily long time to process power-up and power-down and will indicate completion when the read value of this bit matches what was written. |RW |0
|1 |trAtbOutEnable |1=ATB Out Sink enabled. Setting trAtbOutEnable to 0 flushes any queued trace data to ATB.|RW |0
|2 |Reserved |--|--|0
|3 |trAtbOutEmpty |Reads 1 when ATB Out internal buffers are empty |R |1
|7-4 | |Reserved |-- |--
|14-8 |trAtbOutID |ID of this node on ATB. Values of 0x00 and 0x70-0x7F are reserved by the ATB specification and should not be used. |RW |0
|===

*Register: atbOutBase+0x004 atbOutImpl: ATB Out Sink Implementation Register*

//[cols="10%,24%,~,8%,8%",options="header",]
[cols="~,~,~,~,~",options="header",]
|===
|*Bit* |*Field* |*Description* |*RW* |*Reset*
|3-0 |trAtbOutVersion |ATB Out Component Version. Value 1 means module is compliant with this document. |R | 1
|7-4 |trAtbOutCompType |ATB Out Component Type (ATB Out Sink) |R | 0xB(0b1011)
|===
...
* trTeActive - reset for TE block (this will disable encoder from single core/hart)
* trPibActive - reset for PIB block (resets Probe Interface Block only)
* trAtbOutActive - resets ATB Out Sink Block (resets ATB Out Sink Interface)
...
* Enable RAM/PIB/ATB Out sinks in appropriate mode by setting 'tr??Enable=1'
** Wait for confirmation (tr??Active=1)
...
* Start core[s]/hart[s] to be traced (core/hart could be already running as well)
...
* Field trTeInstStallDelta was not present, so debugger may not know if core/hart was ever stalled

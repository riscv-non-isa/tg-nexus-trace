...
*3:* Generate instruction trace using Branch Trace (each taken branch generate trace message). +
*4-5:* Reserved for subset of Branch History Trace. +
*6:* Generate instruction trace using Branch History Trace (each taken or not taken branch adds single history bit). +
...
|9 |trTeInstEnAllJumps|Emit trace message or add history bit for direct conditional jumps. Trace will not be compressed well but timestamp accuracy will be better - maybe be used when profiling loops.|WARL|0
|10 |trTeInstExtendAddrMSB|When set allow extension of virtual address MSB bit (RISC-V Sv39/48/57 virtual address modes assume MSB bits to be all identical). Encoding details are trace protocol dependent.|WARL|0
...
*1-3:* Reserved for future formats
|WARL |SD
|11 |--|Reserved |--|0
...
* Idle sequence (no message/packet to sent) are transmitted between messages.
** Idle sequence depends on trace protocol and must be defined in a way allowing detection of start of first byte of message/packet following idle sequence. 
** Idle sequences are defined in document where trace protocol is defined.

...
2022/12/22: Version 0.9.11-Stable
...
2023/1/9: Version 0.9.12-Stable

* Clarifying PIB Idle Cycles (moved NOTE to separated chapter).
* Added 'trFunnelInputDis' register (allow tracing to more than one destination).
* Notes from Iain (stall-related end typos) taken into account.

2022/12/22: Version 0.9.11-Stable

* Changed RAM Sink SBA mode to SMEM mode (just rename) to not over use SBA term.
* Re-arranged some chapters (for better structure).
* Added chapter describing possible connections between components.
* Added chapter describing minimal requirements for all components (general without going into protocol details).
* All references to registers `highlighted` so are better visible when referenced in text.
* All tables with registers have numbers (for easier reference).
* Added `teTeProtocolMajor/Minor` to be clarified in E-Trace and N-Trace PDFs.
* Simplified 'Legacy version 0 chapter' (removed too hard to maintain list of changes and referring to original PDF donated by SiFive instead)
* Spell checking entire document
...
This document includes chapter 'Minimal Requirements' which include general requirements, but each message protocol supported by this standard must clarify exact meaning of supported registers/fields and bits as some of them define.

There are two standard trace protocols which will utilize this interface:
...
ATB has width (either 8 or 32-bit) which will well match packet=sequence-of-bytes definition.

=== Trace Funnel

The Trace Encoder sends trace messages to a Trace Funnel. The Funnel aggregates trace from each of its inputs (either RISC-V Trace Encoder or another Trace Funnel) and sends the combined trace stream to its designated Trace Sink or ATB Bridge, which is one or more of the sink types above.
...
|1 |trTeEnable |1: Trace Encode is enabled. Allows `trTeInstTracing` and `trTeDataTracing` to turn tracing on and off. Setting `trTeEnable` to 0 flushes any queued trace data to the sink or funnel attached to this encoder. This bit can be set to 1 only by direct writing to it.|RW |0
...
|13 |trTeInstStallEna |
0: If TE cannot send a message, an overflow is generated when the trace is restarted. 
...
|23-20 |trTeSyncMax |The maximum interval (in units determined by `trTeSyncMode`) between synchronization messages/packets. Generate synchronization when count reaches 2^(`trTeSyncMax`+4). If a synchronization packet is generated for another reason, the internal counter should be reset.|WARL |SD
...
NOTE: `trTeProtocol...` fields are separated from `trTeVer...` as we may have the same control interface, but protocol itself may be expanded with new packets/messages fields.
...
|3 |trTeInstEnCallStack|Treat returns as inferable PC discontinuities when returning from a recent call on stack.|WARL|0
...
|6 |trTeDataDropEna|1: Allow dropping data trace to avoid instruction trace overflows. Seting this bit will not guarantee that instuction trace overflows will not happen.|WARL|0
...
In a system with Funnels, typically all the Funnels are built with a Timestamp Unit. The top-level Funnel is the source of the timestamp (Internal System or External) and all the Encoders and other Funnels have a Shared timestamp. To perform the forwarding function, the mid-level Funnels must be programmed with `trFunnelActive` = 1 (which is natural as all trace messages must pass through that funnel).
...
|4 |trRamModeSMEM | 0: This RAM Sink will operate in SRAM mode, 1: This RAM Sink will operate in SMEM mode|WARL |SD
|7-5 |--|Reserved |--|0
...
|31-2 |trRamStartLow |Byte address of start of trace sink circular buffer. It is always aligned on a 32-bit/4-byte boundary. This register may not be implemented if the sink type doesn't require an address. An SRAM sink will usually have `trRamStartLow` fixed at 0. |WARL |Undef or fixed to 0
|===

For bus with an address larger than 32-bit, corresponding `High` registers define the MSB part of such a larger address. 

NOTE: FUTURE: Another extension should deal with signalling (and clearing ...) RAM access errors (especially important for System Bus). Maybe we should have a bit in the `trRamWP` register (where we have `trRamWrap` already) as this register must be read by decoder anyway.
...
|*Bit* |*Field* |*Description* |*RW* |*Reset*
|1-0 |--|Always 0 (two LSB of 32-bit address)|R|0
|31-2 |trRamRPLow |Absolute 32-bit part of address in trace circular memory buffer visible through `trRamData`. `trRamRP` auto-increments following an access to `trRamData`. After a trace word read occurs while `trRamRP` = `trRamLimit`, `trRamRP` is set to `trRamStart`. Required for SRAM mode and optional for SMEM mode. |WARL |0
|===
...
|SMEM Generic |Any (2^N aligned) |Any (`trRamStart` + 2^M - A) - must be set by trace tool |Required  |--|--
|SMEM Fixed   |Fixed (2^N aligned) |Fixed to max size at reset (`trRamStart` + 2^N - A), but can be possibly trimmed |Required  |--|--
|===

NOTE: Value `A` means alignment which depends on memory access width. If we have memory access width of 32-bits, A=4 and value of `trRamLimit` register should be 0x...FC. Some implementations may impose bigger alignment of trace data (to allow more efficient transfer rates). 
...
* Set `trPibEnable`.  This enables the PIB to generate output either immediately (calibration mode) or when the Trace Encoder or Trace Funnel begins sending trace messages.
...
* In 16-bit mode tje first byte of message is transmitted on the LSB part and the MSEO of the second/odd byte will be transmitted on bits #8-#9 and MDO on bits #10-#15.
...
To explain this let's assume the following serially transmitted (in 1-bit PIB mode) sequences of bits (MSEO[0] first on the left):
...
** `tr??Empty` bit must read as 0 when the trace component has some trace data internally buffered.
** `tr??VerMajor`, `tr??VerMinor` and `tr??CompType` must be implemented.

*Minimal Trace Encoder Requirements*

Data-only trace is not considered as a minimal requirement.

* Bit `trTeInstTracing` must be implemented (to start/stop output from Trace Encoder).
* At least `trTeInstMode` = 3 (basic Brach Trace) or 6 (basic History Trace) must be implemented (as hard code-value).
...
*Minimal Trace RAM Sink Requirements*

SRAM mode only:

* Bit `trRamHasSRAM` must be tied to 1 and `trRamModeSMEM` must be tied to 0. 
* Bit `trRamWrap` must be implemented.
* Register `teRamLimitLow` must be implemented, but can be hard coded to value '2^M-4' (address 0x..FC).
* Register `trRapWPLow` must at least accept a write of 0.
* Register `trRamRPLow` must accept any 32-bit aligned value in range <0 .. `teRamLimitLow`> inclusive.
...
* Bit `trRamHasSMEM` must be tied to 1 and `trRamModeSMEM` must be tied to 1.
* Bit `trRamWrap` must be implemented.
* Register `teRamStart` must be implemented, but can be hard coded to value '2^N' (address 0x..00).
* Register `teRamLimit` must be implemented, but can be hard coded to value '2^N + 2^M-4' (address 0x..FC).
* Registers `trRapWP`, `trRamRP` must accept any 32-bit aligned value in inclusive range <`teRamStart` .. `teRamLimit`>.
** If width of access to memory is wider than 32-bits any 32-bit aligned value of `trRamRP` must be allowed and reads must be buffered.
* All other registers/fields/bits may be tied to 0.

*Minimal Trace PIB Sink Requirements*
...
*Minimal ATB Bridge Requirements*

...
* Added 'trTeInstMode' = 4 to allow optimized options to be (potentially ...) used for simpler 'Branch Trace' mode.
* Small notes from Jay (direct email to me).
* Updated wrong names in Filter section (email from Michael) and also changed 'register.field' to 'field' as fields are unique. It makes formatting in narrow columns much nicer.

...
NOTE: RC: What about renaming 'teSink...' into 'teRamSink...'
...
|0x0008 |teInstFeatures |-- |Optional (added in teVersion=1)|Extra instruction trace encoder features
|0x000C |teDataTrace |-- |Optional (added in teVersion=1)|Data trace control and features
...
Branch Trace Messaging is the simplest form of instruction trace. Each discontinuity results in one trace message, either a Direct or Indirect Branch Message. Linear instructions (or sequences of linear instrucions) do not result in any trace messages/packets.
...
The Trace Encoder transmits completed messages to a Trace Sink. This proposal defines a number of different sink types, all optional, and allows an implementation to define other sink types. A Trace Encoder must have at least one sink attached to it.
...
NOTE: Some fields in this interface are different depending on teImpl.teVersion field. 

*Version 0:* Strictly compatible with original SiFive proposal and Nexus implementation. 

*Version 1:* Adjustments to encoder control (in teControl register)

* Some fields renamed (to better reflect meaning and scope).

* Clarified teInstMode and adding teInstFeatures register.

* Field iTrigEnable added (global enable/disable for instruction trace triggering).
...
|2 |iTracing |1=Trace is being generated. Written from tool or controlled by triggers. When teTracing=1, trace data may be subject to additional filtering in some implementations (reserved teInstruction modes). |RW |0
|3 |teEmpty |Reads as 1 when all generated trace has been emitted. |R |1
|6-4 |teInstMode |
Marin instruction trace generation mode

0 = Instruction trace is disabled

1-2 = Reserved for subset of Basic Branch Trace

3 = Generate Branch Trace (each branch generate trace)

4-5 = Reserved for subset of Branch History Trace

6 = Generate non-optimized Branch History Trace (each branch adds single history bit)

7 = Optimized Instruction Trace (teInstFeatures register if present define instruction trace features and optimizations)

|WARL |SD^(1)^
|12-7 |-- |Vendor-specific controls |WARL |SD
|11 |iTrigEnable (only for teVersion=1 or newer) |Global enable/disable for instruction trace triggers|WARL |0
|12 |teStallDelta (only for teVersion=1 or newer) |Read as 1 if stall happened. Clears to 0 on reading.|R |0
|13 |teStallEnable |
...
|15 |teInhibitSrc |1=Disable source field in trace messages. Unless disabled, a trace source field (of teImpl.nSrcBits) is added to every trace message to indicate which TE generated each message. |WARL |SD

|19-16 |teSyncMaxBTM (only for legacy teVersion=0)|Maximum number of trace messages between periodic Sync messages. A Sync emitted for another reason will reset this timer. Generate Sync after 2^(teSyncMaxBTM + 5) trace messages. |WARL |SD
|17-16 |teSyncMode (only for teVersion=1 or newer)|Select periodic synchronization mechanism. At least one non-zero mechanism must be implemented.
...
|19-18 |Reserved (only for teVersion=1 or newer)|--|--|0
|23-20 |teSyncMaxInst (only for legacy teVersion=0)|Maximum instruction unit count between Sync messages. Generate Sync when count reaches 2^(teSyncMaxInst + 4) instruction units (halfwords). |WARL |SD
|23-20 |teSyncMax (only for teVersion=1 or newer)|Thes maximum interval (in units determined by teSyncMode) between synchronization messages/packets. Generate synchronization when count reaches 2^(teSyncMax + 4). If synchronization packet is generated from another reason internal counter should be reset.|WARL |SD
|26-24 |teFormat a|
...
|===
|*Bit* |*Field* |*Description* |*RW* |*Reset*
|3-0 |teVersion |TE Version. See Versions chapter.|R |SD
...
*Register: 0x0008 teInstFeatures: Trace Instruction Features Register (for teImp.version == 1 or bigger)*

NOTE: RC: Below is list of fields taken 'as-is' from chapter 2.2 Optional Modes in E-Trace specification. These should be unified and made 'common'.

[cols=",,,,",options="header",]
|===
|*Bit* |*Field* |*Description* |*RW* |*Reset*

|0|FullAddress|Send only full (non-differential) addresses when set|WARL|0

|1|ImplicitExcept|When set, do not send exception address, only exception cause in Exception packets|WARL|0

|2|siJump|Do not treat sequentially inferrable jumps as un-inferable PC discontinuities when set.|WARL|0

|3|ImplicitReturn|Do not treat returns as uninferable PC discon-tinuities when set.|WARL|0

|4|BranchPrediction|Branch predictor enabled when set.|WARL|0

|5|JumpTargetCache|Jump target cache enabled when set.|WARL|0
...
NOTE: RC: Some fields below are based on Chapter 2.2 Optional Modes in current E-Trace specification. Some fields are added for uniformity with instruction trace.

[cols=",,,,",options="header",]
|===
|*Bit* |*Field* |*Description* |*RW* |*Reset*

|0|dImplemented|Read as 1 if data trace is implemented.|R|SD
|1|dEnable|Main enable for data trace.|RW|0
|2|dTracing |1=Data trace is being generated. Written from tool or controlled by triggers. When dTracing=1, datatrace  may be subject to additional filtering in some implementations.|RW |0
|3||Reserved|--|--
|4|dStallDelta|Set to 1 if data trace caused stall since last read. It is clear on read.|R|0
|5|dStallEnable|Stall execution if data trace message cannot be generated.|WARL|0
|6|dDropDelta|Set to 1 if data trace was dropeed since last read. It is clear on read.|R|0
|7|dAllowDrop|Allow dropping data trace to avoid instruction trace overflows. Seting this bit will not guarantee that instuction trace overflows will not happen.|WARL|0
|15-8||Reserved for additional data trace control/status bits.|--|--
|16|dNoData|Omit data values from data trace packets when set.|WARL|SD
|17|dNoAddress|Omit data address from data trace packets when set.|WARL|SD
|18|dAddrXorMode|Compress data addresses in XOR mode (only LSB bits changed)|WARL|SD
|19|dAddrDiffMode|Compress data addresses in differental mode (+-N offset)|WARL|SD
|20-31|Reserved for more dFeatures|Bit-mask of allowed/enabled data trace features.|WARL|SD
|===

= Trace RAM Sink Control Interface =

NOTE: RC: Thisis new section with 'teSink...' register. I propose to rename these to 'teRamSink...' and add High-option (recently discussed)

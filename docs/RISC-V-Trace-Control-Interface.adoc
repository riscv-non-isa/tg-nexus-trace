...
* Memory accesses through RISC-V debug module using SBA (System Bus Access) as defined in RISC-V Debug Specification. Using SBA access (via debug module) ensures that trace registers are accessible while core/hart is running. It allows external debugger to monitor trace status and to control trace captures without stopping the core/hart being traced.

...
This specification defines the following trace components (*_<N>* in a symbol name denotes 0-based index of particular component)

.*Trace Components*
[cols="40%,30%,40%",options="header",]
|===
|*Component Name* |*Component Type (value=symbol)*|*Base Address (symbol)*
|Trace Encoder |0x1=TRCOMP_ENCODER|trBaseEncoder_<N>
|Trace Funnel |0x8=TRCOMP_FUNNEL|trBaseFunnel_<N>
|Trace RAM Sink |0x9=TRCOMP_RAMSINK|trBaseRamSink_<N>
|Trace PIB Sink |0xA=TRCOMP_PIBSINK|trBasePibSink_<N>
|Trace ATB Bridge |0xE=TRCOMP_ATBBRIDGE|trBaseAtbBridge_<N>
|===
...
|0x044 - 0x07F |--|Reserved |Reserved for bigger read buffer
|===

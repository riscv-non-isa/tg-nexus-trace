...
See <<Implicit Return Optimization,Implicit Return Optimization>> chapter for details.
[[trTeInstEnAllJumps]]
|trTeInstEnAllJumps|1|When set also direct unconditional jumps will be treated as if they were direct conditional (always taken) branches. Trace compression will be significantly affected but using this mode will allow more frequent timestamps, what may be useful for profiling.
...
The Nexus standard provides a lot of flexibility and in general N-Trace can be considered also fully compatible. There is one incompatible, small changes:

...
This chapter describes how fields and bits from Trace Encoder control registers are influencing N-Trace protocol. Only N-Trace specific details and clarifications are provided - all other details are described in <<RISC-V_Trace_Control_Interface,RISC-V Trace Control Interface>> Specification.

IMPORTANT: Registers/fields/bits which are not mentioned in the table below are either generic or are not applicable to this version of N-Trace standard.

[#Details_Control_Parameters]
.Trace Encoder Parameters and Controls
[cols="30%,10%,60%",options="header",]
|======================================================================================================
|Trace Control Field/Bit|Applicability|Description
[[trTeProtocolMajor]]
...
|trTeInstMode|*Required*|N-Trace encoder must support at least one of the following values: +
*3:* <<mode_BTM,BTM>> (Branch Trace Messaging) mode +
*6:* <<mode_HTM,HTM>> (History Branch Messaging) mode +
See <<Main N-Trace Trace Modes,Main N-Trace Trace Modes>> chapter for details.
[[trTeContext]]
|trTeContext|Optional|Controls generation of <<msg2_Ownership,Ownership>> messages.
[[trTsEnable]]
|trTsEnable|Optional|Controls generation of <<field_TSTAMP,TSTAMP>> field.
[[trTeInhibitSrc]]
...
[[trTeInstSyncMode]]
|trTeInstSyncMode|Optional|Controls generation of <<Synchronization Messages,Synchronization Messages>> (with <<field_SYNC,SYNC>> field=2).
[[trTeInstSyncMax]]
|trTeInstSyncMax|Optional|Controls generation of <<Synchronization Messages,Synchronization Messages>> (with <<field_SYNC,SYNC>> field=2).
...
[[trTeInstEnAllJumps]]
|trTeInstEnAllJumps|Optional|As described in <<RISC-V_Trace_Control_Interface,RISC-V Trace Control Interface>> Specification.
|======================================================================================================

NOTE: Above table does not provide names of Trace Encoder control registers as names of bits/fields used in Trace Control Interface are unique.

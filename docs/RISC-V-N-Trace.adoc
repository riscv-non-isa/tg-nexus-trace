...
[[trTeInstSyncMode]]
|trTeInstSyncMode|2|Select the criteria for the periodic generation of instruction trace <<Synchronization Messages,synchronization messages>> (with <<field_SYNC,SYNC>>=2).
[[trTeInstSyncMax]]
|trTeInstSyncMax|4|Configure the maximum interval (in units determined by <<trTeInstSyncMode,trTeInstSyncMode>>) between instruction trace <<Synchronization Messages,synchronization messages>> (with <<field_SYNC,SYNC>>=2).
...
|5| Trace Enable |  Yes | No | Generated when trace is re-enabled after a gap caused by trace being disabled.

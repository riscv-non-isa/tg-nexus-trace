...
|===========================================================================================
| Message/Field:         |TCODE|SRC|SYNC|BTYPE|Other(rare)       |ICNT|xADDR|HIST|TSTAMP|Level
|*Fld Size [bits]:* |[6]  |[M]|[4] |[2]  |     (different)       |[v] |[v]  |[v] |[v]   |
...
|`ProgramTraceSync`      |9    |Opt|Yes |     |                  |Yes |FADDR|    | Opt  |1
...
|`ProgramCorrelation`    |33   |Opt|    |     |EVCODE[4]+ CDF[2] |Yes |     |Opt | Opt  |1,3
|===========================================================================================

*Fld Size:* [n] - fixed size field n-bit, [v] - variable size field, [M] - SRC is only for multi-hart/core tracing

*Levels:*	1 (basic), 2 (optional), 3 (branch history suport), 4 (adds repeated branch for better compression of long loops)
...
. If tracing was disabled and is restarted 'Program Trace Sync' message is generated.
. Any retired instuction increments ICNT (+1 or +2).
.. In case ICNT overflows, Resource Full message (with ICNT before overflow) is generated and ICNT is reset to 1 or 2.
. If this is linear instruction, nothing else is done.
.. In case this is direct/inferable jump (into fixed address), nothing else is done.
.. Not taken branch in BTM mode is also considered as an linear instruction.
. Branch (conditional) either generates DirectBranch (if taken) or adds single bit to branch history buffer. 
.. In case HIST overflows, Resource Full message (with HIST before overflow) is generated and HIST is trimmed to last bit.
...
void NTraceEncoderHandleRetired(uint64_t addr, uint32_t info)
{
    ...
    {
        if (enco_opt_branch_history)
            encoNextEmit = NEXUS_TCODE_IndirectBranchHistory;   // Emit destination address (next retired)
        else

...
static int usage(const char *err)
{
  ...
  printf("\n");
  printf("Usage:\n");
  printf("  NexRvConv -objdump - create info-file from objdump -d call\n");
  printf("  NexRvConv -bin4 <bin> -nex <out> - convert 4-bit file\n");
  return 1;
}
...
int main(int argc, char *argv[])
{
  ...
  if (strcmp(argv[1], "-bin4") == 0)
  {
    if (argc < 5) return error("Incorrect number of parameters");
    if (strcmp(argv[3], "-nex") != 0) return error("-nex must be provided");

    FILE *fIn = fopen(argv[2], "rb");
    if (fIn == NULL) return error("Cannot open bin-file");
    FILE *fOut = fopen(argv[4], "wb");
    if (fOut == NULL)  return error("Cannot create nex-file");

    int ret = ConvBin4(fIn, fOut);
    ...
    if (ret > 0)
    {
      printf(".end ; Converted OK (%d bytes)\n", ret);
      ret = 0;
    }
    else
    {
